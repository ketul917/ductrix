...
################################################################################
    ...
    def create_folder(self, folder):
        ...
        parent = self.get_container_view([vim.Folder],
                parentfolder, limit = {'type': 'Datacenter', 'name': self.datacenter.name })
...
################################################################################
    ...
    def create_vm(self, guest, spec, devices = None ):
        ...
        try:
            ...
            folder = self.get_container_view([vim.Folder], name=foldernm, 
...
#        deploy_ovf1.main( objectview(args), self.si)
        ...
        # Find pool 
        compute = self.get_container_view([vim.ComputeResource], guest['compute_resource'])
        # Get datastore object.
        datastore_list = self.datacenter.datastoreFolder.childEntity
        if guest['datastore_name']:
            datastore_obj = self.get_container_view([vim.Datastore], guest['datastore_name'])
...
#        else:
        ...
        folder = self.get_container_view([vim.Folder], name=self.datacenter.vmFolder.name)
        try:
            parentfolder = self.module.params.get('folder', self.datacenter.vmFolder.name)
...
#            foldernm = guest.get('folder', self.datacenter.vmFolder.name)
        ...
        spec_params = vim.OvfManager.CreateImportSpecParams( entityName=guest['name'], diskProvisioning=guest['disk_type'])
        import_spec = manager.CreateImportSpec(ovfd, rp, datastore_obj, spec_params)
        ...
        while(True):
            if (task.state == vim.HttpNfcLease.State.ready):
                ...
                url = task.info.deviceUrl[0].url.replace('*', self.vsphere_host)
                # Spawn a dawmon thread to keep the lease active while POSTing
                # VMDK.
                keepalive_thread = Thread(target=self.keep_task_alive, args=(task,))
                keepalive_thread.start()
                # POST the VMDK to the host via curl. Requests library would work
                # too.
                curl_cmd = ("curl -Ss -X POST --insecure -T %s -H 'Content-Type: application/x-vnd.vmware-streamVmdk' %s" % (guest['vmdk_file'], url))
                returncode = os.system(curl_cmd)
                if returncode == 0:
                    task.HttpNfcLeaseComplete()
                    keepalive_thread.join()
                    return False, dict(changed = True, msg = "image deploy completed sucessfully" )
                else:
                    task.HttpNfcLeaseAbort()
                    keepalive_thread.join()
                    return True, dict(changed = True, msg = "failed to upload the VMDK to ESXi Host, URL = {0}".format(curl_cmd) )

            elif (task.state == vim.HttpNfcLease.State.error):
                raise Exception("Error connecting to the esxi host to upload vmdk {0}".format(task.error))
        #    break
...
def core(module):
    ...
    diskspec = module.params.get('add_disk', None)
    get_file = module.params.get('get_file', None)
    spec_param = module.params.get('spec', {})
    spec = copy.deepcopy(spec_param)
    folder = module.params.get('folder', None)
    #folder = guest['folder']
    v = Vsphere(module)
    #folderexists = v.get_container_view([vim.Folder], name=folder['name'])
    ...
    if diskspec is not None:
        return v.add_disk(vm, disk_size=diskspec['disk_size'], disk_type=diskspec['disk_type'])
    ...
    if not exists:
        if guest_state == 'present':
            ...
            elif guest.get( 'ovf_file', None ) is not None:
                return v.deploy_ovf(guest, spec)
            else:
